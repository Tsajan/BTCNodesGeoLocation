...
def sniff_addr_packets(host, port):
	...
	print("Sleeping now for 15 seconds")
	time.sleep(15)
	...
	if(conn_established):
		...
		end_time = time.time() + 90 # we want to look up a node upto 90 seconds to see if it contains a list of nodes
		...
		for pkt in capture:
			...
			# stop packet capture if over 90 seconds from an IP
