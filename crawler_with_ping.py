...
def analyze_pkts(reachable_nodes):
	print("Packet Analysis Proceeding!")
	conn_established = False

	for host,port in reachable_nodes.items():
		try:
			print("-------------------------------------------------------")
...
if __name__ == '__main__':
	...
	while(True):
		...
		if(len(nodelistread) >= len(nodelist) ):
			break
